...
var SourceToAsanaMap = module.exports = aei.ideal.Proto.extend().setType("SourceToAsanaMap").newSlots({
	...
	db: function() {
		if (!this._db) {
			...
			this._db.run("CREATE TABLE map(sourceId BIGINTEGER, asanaId BIGINTEGER, UNIQUE(sourceId))");
			this._db.run("CREATE INDEX map_sourceId ON map(sourceId)");
		}
		return this._db;
	},
	...
	atPut: function(sourceId, asanaId) {
		this.db().run("INSERT INTO map(sourceId, asanaId) VALUES(?,?)", [sourceId, asanaId]);
	}
});
