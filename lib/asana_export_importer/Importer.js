...
var Importer = module.exports = aei.ideal.Proto.extend().setType("Importer").newSlots({
	...
	_addUsersToTeams: function() {
		var self = this;
		console.log("adding users to teams ...");

		var futuresBatch = aei.FuturesBatch.clone();
		this.export().teams().forEach(function(team) {
			team.sourceMemberIds().map(self._userAsanaIdWithSourceId.bind(self)).forEach(function (memberAsanaId) {
	...
	_addUsersToProjects: function() {
		...
		this.export().projects().forEach(function(project) {
			var memberAsanaIds = project.sourceMemberIds().map(self._userAsanaIdWithSourceId.bind(self));
	...
	_addUsersToTasks: function() {
		...
		while (task = this.export().taskCursor().next()) {
			if (task.sourceAssigneeId()) {
				futuresBatch.add(task.assignToUser(self._userAsanaIdWithSourceId(task.sourceAssigneeId())));
			}

			var followerAsanaIds = task.sourceFollowerIds().map(self._userAsanaIdWithSourceId.bind(self));
		...
		console.log("added users to tasks.\n");
	},

	_userWithSourceId: function(sourceId) {
		return this._users.detectSlot("sourceId", sourceId);
	},
