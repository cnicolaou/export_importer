	...
	_addUsersToTasks: function() {
		...
		while (task = this.export().taskCursor().next()) {
			if (task.sourceAssigneeId()) {
				futuresBatch.add(task.assignToUser(self._userWithSourceId(task.sourceAssigneeId()).asanaId()));
			}

			var followerAsanaIds = task.followerSourceUserIds().map(function(sourceId){ return self._userWithSourceId(sourceId).asanaId() });
